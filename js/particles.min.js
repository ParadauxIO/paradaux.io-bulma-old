var pJS = function (e, t) {
    var a = "ABCDEFGHIJKLMNOPQRSTUWXYZ", i = document.querySelector("#" + e + " > .particles-js-canvas-el");
    this.pJS = {
    ...
    t && Object.deepExtend(n, t), n.tmp.obj = {
        ...
        if (this.index = Math.floor(Math.random() * a.length), n.canvas.ctx.font = "12px Arial", this.letter = a[this.index], this.radius = (n.particles.size.random ? Math.random() : 1) * n.particles.size.value, n.particles.size.anim.enable && (this.size_status = !1, this.vs = n.particles.size.anim.speed / 100, n.particles.size.anim.sync || (this.vs = this.vs * Math.random())), this.x = i ? i.x : Math.random() * n.canvas.w, this.y = i ? i.y : Math.random() * n.canvas.h, this.x > n.canvas.w - 2 * this.radius ? this.x = this.x - this.radius : this.x < 2 * this.radius && (this.x = this.x + this.radius), this.y > n.canvas.h - 2 * this.radius ? this.y = this.y - this.radius : this.y < 2 * this.radius && (this.y = this.y + this.radius), n.particles.move.bounce && n.fn.vendors.checkOverlap(this, i), this.color = {}, "object" == typeof e.value) if (e.value instanceof Array) {
        ...
        switch (n.canvas.ctx.fillStyle = i, n.canvas.ctx.beginPath(), e.shape) {
            case"random-letter":
                letter = e.letter, n.canvas.ctx.fillText(letter, e.x - 5, e.y + 5);
                break;
            case"circle":
            ...
            case"star":
                ...
                break;
            case"image":
                if ("svg" == n.tmp.img_type) var s = e.img.obj; else s = n.tmp.img_obj;
        ...
        for (var e = 0; e < n.particles.array.length; e++) {
            ...
            if (n.particles.opacity.anim.enable && (1 == t.opacity_status ? (t.opacity >= n.particles.opacity.value && (t.opacity_status = !1), t.opacity += t.vo) : (t.opacity <= n.particles.opacity.anim.opacity_min && (t.opacity_status = !0), t.opacity -= t.vo), t.opacity < 0 && (t.opacity = 0)), n.particles.size.anim.enable && (1 == t.size_status ? (t.radius >= n.particles.size.value && (t.size_status = !1), t.radius += t.vs) : (t.radius <= n.particles.size.anim.size_min && (t.size_status = !0), t.radius -= t.vs), t.radius < 0 && (t.radius = 0)), "bounce" == n.particles.move.out_mode) var i = {
                x_left: t.radius,
                x_right: n.canvas.w,
                y_top: t.radius,
                y_bottom: n.canvas.h
            }; else i = {x_left: -t.radius, x_right: n.canvas.w + t.radius, y_top: -t.radius, y_bottom: n.canvas.h + t.radius};
            switch (t.x - t.radius > n.canvas.w ? (t.x = i.x_left, t.y = Math.random() * n.canvas.h) : t.x + t.radius < 0 && (t.x = i.x_right, t.y = Math.random() * n.canvas.h), t.y - t.radius > n.canvas.h ? (t.y = i.y_top, t.x = Math.random() * n.canvas.w) : t.y + t.radius < 0 && (t.y = i.y_bottom, t.x = Math.random() * n.canvas.w), n.particles.move.out_mode) {
                case"bounce":
                    (t.x + t.radius > n.canvas.w || t.x - t.radius < 0) && (t.vx = -t.vx), (t.y + t.radius > n.canvas.h || t.y - t.radius < 0) && (t.vy = -t.vy)
            }
            if (isInArray("grab", n.interactivity.events.onhover.mode) && n.fn.modes.grabParticle(t), (isInArray("bubble", n.interactivity.events.onhover.mode) || isInArray("bubble", n.interactivity.events.onclick.mode)) && n.fn.modes.bubbleParticle(t), (isInArray("repulse", n.interactivity.events.onhover.mode) || isInArray("repulse", n.interactivity.events.onclick.mode)) && n.fn.modes.repulseParticle(t), n.particles.line_linked.enable || n.particles.move.attract.enable) for (var s = e + 1; s < n.particles.array.length; s++) {
                ...
                n.particles.line_linked.enable && n.fn.interact.linkParticles(t, r), n.particles.move.attract.enable && n.fn.interact.attractParticles(t, r), n.particles.move.bounce && n.fn.interact.bounceParticles(t, r)
            }
        }
        ...
        if (Math.sqrt(a * a + i * i) <= n.particles.line_linked.distance) {
            var s = a / (1e3 * n.particles.move.attract.rotateX), r = i / (1e3 * n.particles.move.attract.rotateY);
        ...
        n.tmp.pushing = !0;
        for (var a = 0; a < e; a++) n.particles.array.push(new n.fn.particle(n.particles.color, n.particles.opacity.value, {
        ...
        if (n.interactivity.events.onhover.enable && isInArray("bubble", n.interactivity.events.onhover.mode)) {
            var t = e.x - n.interactivity.mouse.pos_x, a = e.y - n.interactivity.mouse.pos_y,
                i = 1 - (l = Math.sqrt(t * t + a * a)) / n.interactivity.modes.bubble.distance;

            function s() {
                e.opacity_bubble = e.opacity, e.radius_bubble = e.radius
            }

            if (l <= n.interactivity.modes.bubble.distance) {
                if (i >= 0 && "mousemove" == n.interactivity.status) {
                    if (n.interactivity.modes.bubble.size != n.particles.size.value) if (n.interactivity.modes.bubble.size > n.particles.size.value) {
                        (c = e.radius + n.interactivity.modes.bubble.size * i) >= 0 && (e.radius_bubble = c)
                    } else {
                        var r = e.radius - n.interactivity.modes.bubble.size, c = e.radius - r * i;
                        e.radius_bubble = c > 0 ? c : 0
                    }
                    var o;
                    if (n.interactivity.modes.bubble.opacity != n.particles.opacity.value) if (n.interactivity.modes.bubble.opacity > n.particles.opacity.value) (o = n.interactivity.modes.bubble.opacity * i) > e.opacity && o <= n.interactivity.modes.bubble.opacity && (e.opacity_bubble = o); else (o = e.opacity - (n.particles.opacity.value - n.interactivity.modes.bubble.opacity) * i) < e.opacity && o >= n.interactivity.modes.bubble.opacity && (e.opacity_bubble = o)
                }
            } else s();
            ...
            if (n.tmp.bubble_clicking) {
                t = e.x - n.interactivity.mouse.click_pos_x, a = e.y - n.interactivity.mouse.click_pos_y;
                var l = Math.sqrt(t * t + a * a), v = ((new Date).getTime() - n.interactivity.mouse.click_time) / 1e3;
                v > n.interactivity.modes.bubble.duration && (n.tmp.bubble_duration_end = !0), v > 2 * n.interactivity.modes.bubble.duration && (n.tmp.bubble_clicking = !1, n.tmp.bubble_duration_end = !1)
            }

            function p(t, a, i, s, r) {
                if (t != a) if (n.tmp.bubble_duration_end) null != i && (o = t + (t - (s - v * (s - t) / n.interactivity.modes.bubble.duration)), "size" == r && (e.radius_bubble = o), "opacity" == r && (e.opacity_bubble = o)); else if (l <= n.interactivity.modes.bubble.distance) {
                    if (null != i) var c = i; else c = s;
                    if (c != t) {
                        var o = s - v * (s - t) / n.interactivity.modes.bubble.duration;
            ...
            n.tmp.bubble_clicking && (p(n.interactivity.modes.bubble.size, n.particles.size.value, e.radius_bubble, e.radius, "size"), p(n.interactivity.modes.bubble.opacity, n.particles.opacity.value, e.opacity_bubble, e.opacity, "opacity"))
        }
        ...
        if (n.interactivity.events.onhover.enable && isInArray("repulse", n.interactivity.events.onhover.mode) && "mousemove" == n.interactivity.status) {
            var t = e.x - n.interactivity.mouse.pos_x, a = e.y - n.interactivity.mouse.pos_y, i = Math.sqrt(t * t + a * a),
                s = {x: t / i, y: a / i},
                r = clamp(1 / (o = n.interactivity.modes.repulse.distance) * (-1 * Math.pow(i / o, 2) + 1) * o * 100, 0, 50),
                c = {x: e.x + s.x * r, y: e.y + s.y * r};
            "bounce" == n.particles.move.out_mode ? (c.x - e.radius > 0 && c.x + e.radius < n.canvas.w && (e.x = c.x), c.y - e.radius > 0 && c.y + e.radius < n.canvas.h && (e.y = c.y)) : (e.x = c.x, e.y = c.y)
            ...
            var o = Math.pow(n.interactivity.modes.repulse.distance / 6, 3), l = n.interactivity.mouse.click_pos_x - e.x,
                v = n.interactivity.mouse.click_pos_y - e.y, p = l * l + v * v, d = -o / p * 1;
            p <= o && function () {
                var t = Math.atan2(v, l);
                if (e.vx = d * Math.cos(t), e.vy = d * Math.sin(t), "bounce" == n.particles.move.out_mode) {
                    var a = {x: e.x + e.vx, y: e.y + e.vy};
                    (a.x + e.radius > n.canvas.w || a.x - e.radius < 0) && (e.vx = -e.vx), (a.y + e.radius > n.canvas.h || a.y - e.radius < 0) && (e.vy = -e.vy)
                }
            }()
        ...
        "window" == n.interactivity.detect_on ? n.interactivity.el = window : n.interactivity.el = n.canvas.el, (n.interactivity.events.onhover.enable || n.interactivity.events.onclick.enable) && (n.interactivity.el.addEventListener("mousemove", (function (e) {
            if (n.interactivity.el == window) var t = e.clientX, a = e.clientY; else t = e.offsetX || e.clientX, a = e.offsetY || e.clientY;
        ...
        if (n.particles.number.density.enable) {
            ...
            var t = e * n.particles.number.value / n.particles.number.density.value_area, a = n.particles.array.length - t;
        ...
        for (var a = 0; a < n.particles.array.length; a++) {
            var i = n.particles.array[a], s = e.x - i.x, r = e.y - i.y;
        ...
        var t = n.tmp.source_svg.replace(/#([0-9A-F]{3,6})/gi, (function (t, a, i, n) {
                ...
                return s
            })), a = new Blob([t], {type: "image/svg+xml;charset=utf-8"}), i = window.URL || window.webkitURL || window,
            s = i.createObjectURL(a), r = new Image;
        ...
        if (n.tmp.img_error = void 0, "" != n.particles.shape.image.src) if ("svg" == e) {
...
Object.deepExtend = function (e, t) {
    ...
    var a = document.getElementById(e), i = a.getElementsByClassName("particles-js-canvas-el");
    ...
    }, window.getAnchor = function (e) {
        var t = e.split("#");
        return null == t || t.length < 2 ? "" : ["#", t[1]].join("")
    }, window.go_to_top = function () {
        window.emailField.focus()
    }, window.scrollToMainSection = () => {
        var e = $('[data-class="go-to-menu-sec"]');
        0 !== e.length && e[0].scrollIntoView({behavior: "smooth"})
    };
    var e = "rgb(242, 242, 242)" == getComputedStyle(document.body)["background-color"] ? "light" : "dark",
        t = "light" == e ? "/js/json/particles.json" : "/js/json/particles-dark.json";
    try {
        particlesJS.load("particles-js", t, (function () {
        }))
    } catch (e) {
    }
    tippy("#email-input", {content: "Tooltip", trigger: "click", theme: "light" == e ? "light" : "dark"});
    let a = null;
    $('[data-class^="researchers-button"],     [data-class^="institutions-button"]').click(e => {
        const t = e.target.getAttribute("data-class"), i = e.target.getAttribute("href"), n = t.replace("button", "sec");
        if (i) {
            const e = i.replace("#", "");
            if (-1 !== i.indexOf("#institutions-word")) return void $(`[data-class="${e}"]`)[0].scrollIntoView({behavior: "smooth"})
        }
        if (a == n || (null == a ? $(`[data-class="${n}"]`).slideDown() : ($(`[data-class="${n}"]`).show(), $(`[data-class="${a}"]`).hide())), a = n, i) {
            const e = i.replace("#", "");
            if (-1 !== i.indexOf("#institutions-sec")) return void setTimeout(() => {
                $(`[data-class="${e}"]`)[0].scrollIntoView({behavior: "smooth"})
            }, 500)
        }
        $(`[data-class="${n}"]`)[0].scrollIntoView({behavior: "smooth"})
    }), $('[data-class="go-up-btn"]').click(e => {
        e.preventDefault(), window.scrollToMainSection()
    }), 0 !== getPath().length && setTimeout(() => {
        scrollToMainSection()
    }, 50), $("#word-download-vsto").click((function () {
        amplitude.getInstance().logEvent("download:word-vsto")
    })), $("#word-download-app-source").click((function () {
        amplitude.getInstance().logEvent("download:word-app-source")
    })), $("#revise-upload").click((function () {
        amplitude.getInstance().logEvent("upload:revise")
    })), $("#word-learn-more").click((function () {
        amplitude.getInstance().logEvent("learn-more:word")
    })), $("#revise-learn-more").click((function () {
        amplitude.getInstance().logEvent("learn-more:revise")
    })), $("#revise-try").click((function () {
        amplitude.getInstance().logEvent("try:revise")
    }));
    try {
        var i = window.location.pathname.replace("/", "").replace(".html", "");
        0 === i.length && (i = "index"), amplitude.getInstance().logEvent("visit:" + i)
    } catch (e) {
    }
}();